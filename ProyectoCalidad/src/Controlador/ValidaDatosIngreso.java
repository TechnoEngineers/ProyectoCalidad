...
/**
 * Este metodo es utilizado para la validación de algunas cajas de texto
 * contenidas en clases como Login, RecuperarContrasena, VerificarUsuario,
 * VerificarPreguntas.
 *
 * @author angelcareaga
 */
public class ValidaDatosIngreso
{
     ...
     * sEstructuraValidaEmail; Almacena expresión regular para validar el
     * correo. iCadenaInvalida; Almacena la validación en caso de que la cadena
     * recibida sea invalida. bUsuarioValido; Almacena la validación en caso de
     * que el usuario sea valido. bContraseniaValida; Almacena la validación en
     * caso de que la contraseña sea valida. bCajaContraseniaVacia; Almacena la
     * validación en caso de que la caja contraseña este vacia. bLimpiarCaja;
     * Almacena la validación en caso de que se quiera limpiar las cajas de
     * texto. bCorreoValido; Almacena la validación en caso de que el correo sea
     * valido. sCadenaSinEspacios; Se usa la variable para guardar la cadena sin
     * espacios.
     */
    ...
    /**
     * En este metodo se valida la cadena alfanumerica de Usuario, ciudad y
     * cualquier otro que requiera verificar si es alfanumerica.
     *
     * @param sCadena la cadena es recibida desde su llamado para validarla.
     * @param sTipoValidacion Recibe el tipo de validación, ya sea de usuario,
     * ciudad o cualquier otro, esto para mostrar mensajes correctos.
     * @param sOtraValidacion Recibe otro dato de validación, esto es porque se
     * queriere un auxiliar en la clase VerificarPreguntas, esto ayuda a mostrar
    ...
    public String validaCadenaAlfanumerica(String sCadena, String sTipoValidacion, String sOtraValidacion)
    {
        ...
        /**
         * Declaración de variables a ocupar. iNoAlfanumerico; Cambiará el valor
         * dependiendo si es alfanumerica o no. iAlfanumericoMinuscula; Cambiará
         * el valor dependiendo si a cadena contiene un caracter con letra
         * minuscula. iAlfanumericoMayuscula; Cambiará el valor dependiendo si a
         * cadena contiene un caracter con letra mayuscula.
         * iAlfanumericoNumerico; Cambiará el valor dependiendo si a cadena
         * contiene un caracter con un número. caCadenaArreglo; con la función
         * toCharArray() va a convertir la cadena recibida sin espaciós a un
         * arreglo de caracteres y se van a asignar a esta variable.
         */
        ...
        } else if (sCadenaSinEspacios.length() >= 5 && sCadenaSinEspacios.length() <= 100)
        {
            for (int i = 0; i < sCadenaSinEspacios.length(); i++)
            {
                ...
                } else if (caCadenaArreglo[i] >= 97 && caCadenaArreglo[i] <= 122) //Valida que cada caracter entre en el codico ascii utilizando así caracteres entre el 97 y 122, que son caracteres de letras en minuscula.
                {
                    iAlfanumericoMayuscula = 1;
                } else if (caCadenaArreglo[i] >= 48 && caCadenaArreglo[i] <= 59)//Valida que cada caracter entre en el codico ascii utilizando así caracteres entre el 48 y 59, que son caracteres de numeros.
                {
            ...
            } else if (iAlfanumericoMayuscula == 1 && iAlfanumericoNumero == 1)
            {
            ...
            } else if (iAlfanumericoMinuscula == 1 && iAlfanumericoNumero == 1)
            {
                iCadenaInvalida = 0; //Si es cadena alfanumerica y cumple con los requisitos de usuario entonces la cadena es valida.
                bUsuarioValido = true; //El usuario es valido, asigna el valor de true.
                System.out.println(sTipoValidacion + " correct@");
            ...
            if (sTipoValidacion.equals("respuestasecreta")) //Si el parametro es de ciudad, admite letras y numeros, o solo letras pero no solo numeros.
            {
                ...
                } else if (iAlfanumericoMinuscula == 1)
                {
    ...
    /**
     * En validaContrasenia se valida la contraseña. Se valido que fuera mayor
     * de 4 y menor de 100 caracteres. Si no esta dentro de estos parametros
     * manda error de contraseña invalida. Se valido que la caja donde va la
     * contraseña no mande a validar una cadena vacia, si es así, manda que
     * falta la contraseña,
     *
     * @param sCadena la cadena es recibida desde su llamado para validarla.
    ...
    public String validaContrasenia(String sCadena)
    {
        ...
        /**
         * En el siguiente if se valida que la contraseña este entre 4 y 100
         * caracteres. Si no es así valida que la cadena no sea menor o igual a
         * cero caracteres, si es así, esta vacia, por lo tanto falta la
         * contraseña. Sí ninguna de las anteriores se cumple, entonces es
         * invalida, ya que no entro en el primer if.
         */
    ...
    /**
     * En esta funcion se valida el correo, esto lo hace mediante una expresión
     * regular. También se valida que la caja de correo no este vacía.
     *
     * @param sCadena la cadena es recibida desde su llamado para validarla.
     * @return bCorreoValido regresa el valor si es valido el correo recibido o
     * no.
     */
    public boolean validaCorreo(String sCadena)
    {
        ...
        /**
         * En el siguiente if: Valida que la cadena sin espacios sea menor que
         * cero, si eso pasa manda un mensaje de error, ya que se requiere
         * validar que haya correo y que sea valido. Si no se cumple la
         * condición entra en else.
         */
